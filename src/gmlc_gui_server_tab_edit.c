...
typedef struct _sColInfo {
	GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery;
	GtkWidget * lstSQLResult;
	guint iNumCol;
	guint iRowCol;
} sColInfo;

static void gmlc_gui_server_tab_edit_finalize (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery);
static void gmlc_gui_server_tab_edit_get_property (GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
static void gmlc_gui_server_tab_edit_set_property (GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);

static void gmlc_gui_server_tab_edit_interface_init (gpointer g_iface, gpointer iface_data);
static void gmlc_gui_server_tab_edit_update_ui (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery, gboolean bShow);

void gmlc_gui_server_tab_edit_create_toolbar_items (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery);
void gmlc_gui_server_tab_edit_create_widgets (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery);
void gmlc_gui_server_tab_edit_init_widgets (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery);

void gmlc_gui_server_tab_edit_execute_query (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery, const gchar * query);
void gmlc_gui_server_tab_edit_clean_all_result(GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery);
void gmlc_gui_server_tab_edit_display_all_result(GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery);
void gmlc_gui_server_tab_edit_display_one_result(GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery);
void gmlc_gui_server_tab_edit_display_one_info(GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery);

gboolean gmlc_gui_server_tab_edit_evt_window_keyrelease (GtkWidget *widget, GdkEventKey *event, gpointer user_data);

void gmlc_gui_server_tab_edit_load_from_file (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery, const gchar * pcFileName);
void gmlc_gui_server_tab_edit_save_to_file (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery, const gchar * pcFileName);
void gmlc_gui_server_tab_edit_evt_btn_save_clicked(GtkWidget *widget, gpointer user_data);

void gmlc_gui_server_tab_edit_evt_btnExecSql_clicked(GtkWidget *widget, gpointer user_data);
void gmlc_gui_server_tab_edit_evt_btnDuplicateSql_clicked(GtkWidget *widget, gpointer user_data);
void gmlc_gui_server_tab_edit_evt_btnDumpSql_clicked(GtkWidget *widget, gpointer user_data);
void gmlc_gui_server_tab_edit_evt_btnClose_clicked(GtkWidget *widget, gpointer user_data);

void gmlc_gui_server_tab_edit_evt_btnSaveResult_clicked(GtkWidget *widget, gpointer user_data);
static gboolean ggstqe_btnSaveResult_foreach (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer user_data);

void gmlc_gui_server_tab_edit_evt_resultRow_edited (GtkCellRendererText *cellrenderertext, gchar *path_string, gchar *new_value, gpointer user_data);

static gboolean gmlc_gui_server_tab_edit_evt_resultRow_btnpress (GtkWidget *widget, GdkEventButton *event, gpointer user_data);
static void gmlc_gui_server_tab_edit_evt_resultRow_menu_copy_activate (GtkWidget *widget, gpointer user_data);
static void gmlc_gui_server_tab_edit_evt_resultRow_menu_delete_activate (GtkWidget *widget, gpointer user_data);
...
static void gmlc_gui_server_tab_edit_interface_init (gpointer g_iface, gpointer iface_data) {
  ...
  pIface->update_ui = (void (*) (GmlcGuiServerTab * self, gboolean bShow))gmlc_gui_server_tab_edit_update_ui;
}

static void gmlc_gui_server_tab_edit_class_init (GmlcGuiServerTabEditClass * pClass) {
	GObjectClass *pObjClass = G_OBJECT_CLASS(pClass);
	
	pObjClass->finalize = (GObjectFinalizeFunc) gmlc_gui_server_tab_edit_finalize;
	
	pObjClass->get_property = gmlc_gui_server_tab_edit_get_property;
	pObjClass->set_property = gmlc_gui_server_tab_edit_set_property;
...
static void gmlc_gui_server_tab_edit_get_property (GObject * object, guint prop_id, GValue * value, GParamSpec * pspec) {
	GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery = GMLC_GUI_SERVER_TAB_EDIT(object);
...
static void gmlc_gui_server_tab_edit_set_property (GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec) {
	...
	switch (prop_id) {
		case PROP_SERVER_WINDOW :
			...
			gmlc_gui_server_tab_edit_create_toolbar_items(pGmlcGuiSrvTabQuery);
			gmlc_gui_server_tab_edit_create_widgets(pGmlcGuiSrvTabQuery);
			gmlc_gui_server_tab_edit_init_widgets(pGmlcGuiSrvTabQuery);
...
static void gmlc_gui_server_tab_edit_init (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery) {
	...
	g_signal_connect (G_OBJECT (pGmlcGuiSrvTabQuery), "key-release-event", G_CALLBACK (gmlc_gui_server_tab_edit_evt_window_keyrelease), pGmlcGuiSrvTabQuery);
}
...
GmlcGuiServerTabEdit * gmlc_gui_server_tab_edit_new (GmlcGuiServer * pGmlcGuiSrv, const gchar * pcDefDbName) {
	GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery = NULL;
	
	pGmlcGuiSrvTabQuery = GMLC_GUI_SERVER_TAB_EDIT(g_object_new(GMLC_GUI_TYPE_SERVER_TAB_EDIT, "db_name", pcDefDbName, "server_window", pGmlcGuiSrv, NULL));
...
void gmlc_gui_server_tab_edit_create_toolbar_items (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery) {
	...
	gtk_button_set_relief(GTK_BUTTON(poBtn), GTK_RELIEF_NONE);
	g_signal_connect(poBtn, "clicked", G_CALLBACK (gmlc_gui_server_tab_edit_evt_btnExecSql_clicked), pGmlcGuiSrvTabQuery);
	...
	gtk_button_set_image(GTK_BUTTON(poBtn), poImgBtn);
	gtk_button_set_relief(GTK_BUTTON(poBtn), GTK_RELIEF_NONE);
	g_signal_connect(poBtn, "clicked", G_CALLBACK (gmlc_gui_server_tab_edit_evt_btnDuplicateSql_clicked), pGmlcGuiSrvTabQuery);
	gtk_widget_show(poBtn);
	gtk_box_pack_start(GTK_BOX(pGmlcGuiSrvTabQuery->poQueryToolbar), poBtn, FALSE, FALSE, 0);
...
/*
	...
	gtk_button_set_image(GTK_BUTTON(poBtn), poImgBtn);
	gtk_button_set_relief(GTK_BUTTON(poBtn), GTK_RELIEF_NONE);
/ *	g_signal_connect(poBtn, "clicked", G_CALLBACK (gmlc_gui_server_tab_edit_evt_btnDumpSql_clicked), pGmlcGuiSrvTabQuery); * /
	gtk_widget_show(poBtn);
	gtk_box_pack_start(GTK_BOX(pGmlcGuiSrvTabQuery->poQueryToolbar), poBtn, FALSE, FALSE, 0);
*/
	...
	gtk_button_set_image(GTK_BUTTON(poBtn), poImgBtn);
	gtk_button_set_relief(GTK_BUTTON(poBtn), GTK_RELIEF_NONE);
	g_signal_connect(poBtn, "clicked", G_CALLBACK (gmlc_gui_server_tab_edit_evt_btnClose_clicked), pGmlcGuiSrvTabQuery);
	gtk_widget_show(poBtn);
	gtk_box_pack_start(GTK_BOX(pGmlcGuiSrvTabQuery->poQueryToolbar), poBtn, FALSE, FALSE, 0);
...
void gmlc_gui_server_tab_edit_create_widgets (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery) {
	...
	gtk_paned_pack1 (GTK_PANED (vpanedSQL), scrlwndSQLRequest, TRUE, TRUE);
	...
	gtk_paned_pack2 (GTK_PANED (vpanedSQL), pGmlcGuiSrvTabQuery->tabSQLResult, FALSE, TRUE);
...
gboolean gmlc_gui_server_tab_edit_set_query (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery, const gchar * pcQuery, gboolean bExecNow) {
	...
	if (pcQuery != NULL) {
		...
		if (bExecNow) {
			gmlc_gui_server_tab_edit_execute_query (pGmlcGuiSrvTabQuery, pcQuery);
		}
		return TRUE;
	}
...
void gmlc_gui_server_tab_edit_execute_query (GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery, const gchar * query) {
	...
	gmlc_gui_server_tab_edit_clean_all_result(pGmlcGuiSrvTabQuery);
	
	gmlc_gui_server_tab_edit_display_all_result(pGmlcGuiSrvTabQuery);
}
...
void gmlc_gui_server_tab_edit_display_all_result(GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery) {
	...
	do {
		...
		if (pGmlcGuiSrvTabQuery->pGmlcMysqlQry->iErrCode == 0 && pGmlcGuiSrvTabQuery->pGmlcMysqlQry->iNbField == 0) {
			gmlc_gui_server_tab_edit_display_one_info(pGmlcGuiSrvTabQuery);
		} else {
			gmlc_gui_server_tab_edit_display_one_result(pGmlcGuiSrvTabQuery);
		}
...
void gmlc_gui_server_tab_edit_display_one_result(GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery) {
	...
	GtkWidget * poMenuOps = NULL, * poMenuOpsItem = NULL;
...
*/
	...
	poMenuOps = gtk_menu_new();
	gtk_widget_show (poMenuOps);
	...
	g_signal_connect (G_OBJECT (select), "changed", G_CALLBACK (gmysqlcc_gui_server_tab_edit_evt_resultRow_selected), gui_pGmlcGuiSrvTabQuery);
	*/
	...
	if (gmlc_mysql_query_is_editable(pGmlcGuiSrvTabQuery->pGmlcMysqlQry)) {
		g_signal_connect (poMenuOpsItem, "activate", G_CALLBACK (gmlc_gui_server_tab_edit_evt_resultRow_menu_delete_activate), lstSQLResult);
	} else {
		gtk_widget_set_sensitive(poMenuOpsItem, FALSE);
	}
	...
	if (arHeaders != NULL) {
		...
		if (gmlc_mysql_query_is_editable(pGmlcGuiSrvTabQuery->pGmlcMysqlQry)) {
			...
			for (i = 0; i < arHeaders->len; i++) {
				...
				g_signal_connect (G_OBJECT (ppoRenderer[i]), "edited", G_CALLBACK (gmlc_gui_server_tab_edit_evt_resultRow_edited), (gpointer)&psColInfo[i]);
...
void gmlc_gui_server_tab_edit_evt_btn_load_clicked(GtkWidget *widget, gpointer user_data) {
	GmlcGuiServerTabEdit * pGmlcGuiSrvTabQuery = GMLC_GUI_SERVER_TAB_EDIT(user_data);
	...
	if (response == GTK_RESPONSE_OK) {
		...
		if (pcFileName != NULL) {
			gmlc_gui_server_tab_edit_load_from_file (pGmlcGuiSrvTabQuery, pcFileName);
			g_free (pcFileName);
		}
	}
...
void gmlc_gui_server_tab_edit_evt_btn_save_clicked(GtkWidget *widget, gpointer user_data) {
	...
	if (response == GTK_RESPONSE_OK) {
		...
		pcFileName = gtk_file_chooser_get_filename (GTK_FILE_CHOOSER (poChooser));
		if (pcFileName != NULL) {
			gmlc_gui_server_tab_edit_save_to_file (pGmlcGuiSrvTabQuery, pcFileName);
			g_free (pcFileName);
		}
	}
...
void gmlc_gui_server_tab_edit_evt_btnExecSql_clicked(GtkWidget *widget, gpointer user_data) {
	...
	g_print("Execute SQL\n");
	pcSqlQuery = gmlc_gui_server_tab_edit_get_query(pGmlcGuiSrvTabQuery);
	g_print("SQL Query : '%s'\n", pcSqlQuery);
	
	gmlc_gui_server_tab_edit_execute_query(pGmlcGuiSrvTabQuery, pcSqlQuery);
...
void gmlc_gui_server_tab_edit_evt_btnDuplicateSql_clicked(GtkWidget *widget, gpointer user_data) {
	...
	gmlc_gui_server_add_edit_tab(pGmlcGuiSrvTabQuery->pGmlcGuiSrv, pGmlcGuiSrvTabQuery->pcDbName, pcSqlQuery);
...
void gmlc_gui_server_tab_edit_evt_resultRow_edited (GtkCellRendererText *cellrenderertext, gchar *path_string, gchar *new_value, gpointer user_data) {
	...
	if (tree_model != (GtkTreeModel *)NULL) {
		if (gtk_tree_model_get_iter_from_string (tree_model, &iter, path_string)) {
			gtk_tree_model_get_value(GTK_TREE_MODEL(tree_model), &iter, psColInfo->iRowCol, &sGValPtr);
			poRow = g_value_get_pointer(&sGValPtr);
			g_value_unset(&sGValPtr);
...
static gboolean gmlc_gui_server_tab_edit_evt_resultRow_btnpress (GtkWidget *widget, GdkEventButton *event, gpointer user_data) {
	GtkMenu * poOpsMenu = GTK_MENU(user_data);
...
static void gmlc_gui_server_tab_edit_evt_resultRow_menu_copy_activate (GtkWidget *widget, gpointer user_data) {
	GtkWidget * lstSQLResult = GTK_WIDGET(user_data);
	GtkTreeModel * poTreeModel = NULL;
	GtkTreePath * poTreePath = NULL;
	GtkTreeViewColumn * poTreeColumn = NULL;
	GtkClipboard * poClipboard = NULL;
	GmlcMysqlRow * poRow = NULL;
	GdkAtom oAtom;
	GValue sGValPtr = {0, };
	GtkTreeIter sIter;
	...
	gtk_tree_view_get_cursor(GTK_TREE_VIEW(lstSQLResult), &poTreePath, &poTreeColumn);
	
	poTreeModel = gtk_tree_view_get_model(GTK_TREE_VIEW(lstSQLResult));
	if (poTreeModel != (GtkTreeModel *)NULL) {
		iRowCol = gtk_tree_model_get_n_columns(poTreeModel) - 1;
		if (poTreePath != NULL) {
			if (gtk_tree_model_get_iter(poTreeModel, &sIter, poTreePath)) {
				gtk_tree_model_get_value(GTK_TREE_MODEL(poTreeModel), &sIter, iRowCol, &sGValPtr);
				poRow = g_value_get_pointer(&sGValPtr);
				g_value_unset(&sGValPtr);
				
				if (GMLC_MYSQL_IS_ROW(poRow)) {
					oAtom = gdk_atom_intern("PRIMARY", TRUE);
					poClipboard = gtk_clipboard_get(oAtom);
					gtk_clipboard_set_text(poClipboard, gmlc_mysql_row_get_row_text(poRow), -1);
					g_print("Current line : %s\n", gmlc_mysql_row_get_row_text(poRow));
				}
			}
		}
	}
...
static void gmlc_gui_server_tab_edit_evt_resultRow_menu_delete_activate (GtkWidget *widget, gpointer user_data) {
	...
	GValue sGValPtr = {0, };
	GtkTreeIter sIter;
	gint iRowCol = 0, iRetMessageBox = 0;
	UNUSED_VAR(widget);
	...
	if (poTreeModel != (GtkTreeModel *)NULL) {
		...
		if (poTreePath != NULL) {
			if (gtk_tree_model_get_iter(poTreeModel, &sIter, poTreePath)) {
				...
				if (GMLC_MYSQL_IS_ROW(poRow)) {
					poMessageBox = gtk_message_dialog_new (NULL,
						GTK_DIALOG_DESTROY_WITH_PARENT,
						GTK_MESSAGE_WARNING, GTK_BUTTONS_OK_CANCEL,
						_("Do you want delete this line ?"));
					iRetMessageBox = gtk_dialog_run(GTK_DIALOG (poMessageBox));
					if (iRetMessageBox == GTK_RESPONSE_OK) {
						gmlc_mysql_row_delete(poRow);
						g_print("Delete line : %s\n", gmlc_mysql_row_get_field_value(poRow, 0));
					}
					gtk_widget_destroy(poMessageBox);
				}
			}
		}
	}
