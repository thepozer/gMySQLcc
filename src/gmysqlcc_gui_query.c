...
gchar * gmysqlcc_gui_query_get_current_query (p_gmysqlcc_gui_query gui_query) {
	...
	sqlQuery = (gchar *)gtk_text_buffer_get_text (GTK_TEXT_BUFFER(txtBuffer), &begin, &end, FALSE);
...
/*static void cleanQueryResult(p_execSqlWnd pExecWnd) {*/
	...
	/* Delete old Columns */
	while ((currCol = gtk_tree_view_get_column(GTK_TREE_VIEW(gui_query->lstSQLResult), 0)) != (GtkTreeViewColumn *)NULL) {
...
void gmysqlcc_gui_query_execute (p_gmysqlcc_gui_query gui_query, const char * sqlQuery) {
	p_mysql_query mysql_qry;
	...
	if (!mysql_query_execute_query(mysql_qry, sqlQuery, TRUE)) {
		/* Query Not Ok */
		msgdlg = gtk_message_dialog_new(GTK_WINDOW(gui_query->window), GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, _("Error during the query : (%d) %s"), mysql_qry->errCode, mysql_qry->errMsg);
...
void gmysqlcc_gui_query_evt_key_released (GtkWidget *widget, GdkEventKey *event, gpointer user_data) {
	if (event->state & GDK_CONTROL_MASK && (event->keyval == GDK_e || event->keyval == GDK_E)) {
		gmysqlcc_gui_query_evt_execute_clicked (widget, user_data);
		return TRUE;
	}
...
void gmysqlcc_gui_query_evt_duplicate_clicked (GtkWidget *widget, gpointer user_data) {
	p_gmysqlcc_gui_query gui_query = (p_gmysqlcc_gui_query)user_data;
	p_gmysqlcc_gui_query gui_query_dup;
	gchar * sqlQuery;

	sqlQuery = gmysqlcc_gui_query_get_current_query(gui_query);
	
	gui_query_dup = gmysqlcc_gui_query_new(mysql_query_get_database(gui_query->mysql_qry));
...
void gmysqlcc_gui_query_evt_dump_clicked (GtkWidget *widget, gpointer user_data) {
	p_gmysqlcc_gui_query gui_query = (p_gmysqlcc_gui_query)user_data;
...
void gmysqlcc_gui_query_evt_execute_clicked (GtkWidget *widget, gpointer user_data) {
	p_gmysqlcc_gui_query gui_query = (p_gmysqlcc_gui_query)user_data;
	gchar * sqlQuery;
	...
	gmysqlcc_gui_query_execute(gui_query, sqlQuery);
	
	g_free(sqlQuery);
}

/*static void btnHistory_clicked (GtkWidget *widget, gpointer user_data) {*/
void gmysqlcc_gui_query_evt_history_clicked (GtkWidget *widget, gpointer user_data) {
	/*p_gmysqlcc_gui_query gui_query = (p_gmysqlcc_gui_query)user_data;*/
...
void gmysqlcc_gui_query_evt_close_clicked (GtkWidget *widget, gpointer user_data) {
	p_gmysqlcc_gui_query gui_query = (p_gmysqlcc_gui_query)user_data;
...
void gmysqlcc_gui_query_evt_charset_changed (GtkComboBox *combobox, gpointer user_data) {
	p_gmysqlcc_gui_query gui_query = (p_gmysqlcc_gui_query)user_data;
	gint idx;
	gchar * charset;
	GtkWidget * msgdlg;
	...
	if (!mysql_query_change_charset(gui_query->mysql_qry, charset)) {
		...
		gmysqlcc_gui_query_evt_execute_clicked (GTK_WIDGET(combobox), user_data);
	}
}
...
gboolean gmysqlcc_gui_query_evt_result_row_edited (GtkCellRendererText *cellrenderertext, gchar *path_string, gchar *new_value, gpointer data) {
	GtkTreeIter iter;
...
gboolean gmysqlcc_gui_query_set_query (p_gmysqlcc_gui_query gui_query, const gchar * query, gboolean execNow) {
	...
	if (query != (gchar *) NULL) {
		...
		if (execNow) {
			gmysqlcc_gui_query_execute (gui_query, query);
		}
	}
}
...
p_gmysqlcc_gui_query gmysqlcc_gui_query_new (p_mysql_database mysql_db) {
/*p_execSqlWnd create_wndSQL(gboolean display, p_mysql_query mysql_qry, const gchar * query, gboolean execNow) {*/
  ...
  GtkWidget *scrlwndSQLRequest;
	...
	gui_query->mysql_qry = mysql_database_query(mysql_db);
  ...
  gtk_window_set_title (GTK_WINDOW (gui_query->window), sTitle->str);
	...
	g_signal_connect (G_OBJECT (gui_query->window), "key-release-event", G_CALLBACK (gmysqlcc_gui_query_evt_key_released), gui_query);
  ...
  gtk_toolbar_set_style (GTK_TOOLBAR (tlbSql), GTK_TOOLBAR_BOTH_HORIZ);
	...
	g_signal_connect (G_OBJECT (btnExecSql), "clicked", G_CALLBACK (btnexecsql_clicked), gui_query);
	...
	g_signal_connect (G_OBJECT (pExecWnd->cmbCharset), "changed", G_CALLBACK (cmbCharset_changed), gui_query);
	idx = 0;
	...
	g_signal_connect (G_OBJECT (btnDuplicateSql), "clicked", G_CALLBACK (btnduplicatesql_clicked), gui_query);
	...
	g_signal_connect (G_OBJECT (btnDumpSql), "clicked", G_CALLBACK (btndumpsql_clicked), gui_query);
	...
	g_signal_connect (G_OBJECT (btnHistory), "clicked", G_CALLBACK (btnHistory_clicked), gui_query);
	...
	*/
	...
	g_signal_connect (G_OBJECT (btnClose), "clicked", G_CALLBACK (btnclose_clicked), gui_query);
  ...
  gtk_container_add (GTK_CONTAINER (gui_query->sclSQLResult), gui_query->lstSQLResult);
	...
	g_signal_connect (G_OBJECT (select), "changed", G_CALLBACK (resultRow_selected), gui_query);
